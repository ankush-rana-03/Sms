...
class TeacherService {
  ...
  async getStudentsByClass(studentClass: string, section?: string): Promise<{ success: boolean; count: number; data: Student[] }> {
    try {
      const params = new URLSearchParams({ class: studentClass });
  ...
  async getTodayAttendance(studentClass: string, section?: string): Promise<{
    success: boolean;
    date: string;
    count: number;
